...
export default function JoinTelegramButton({
  ...
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.96 }}
    >
      <span className="telegram-cta-shimmer pointer-events-none absolute inset-0 rounded-full" aria-hidden />
      <motion.span
        className="relative z-10 flex items-center justify-center"
        animate={{ rotate: [0, -8, 8, -4, 0] }}
        transition={{ duration: 2.5, repeat: Infinity, repeatDelay: 1.5, ease: 'easeInOut' }}
      >
        <Send size={iconSize} className={variant === 'filled' ? 'text-white' : 'text-cyan-400'} />
      </motion.span>
